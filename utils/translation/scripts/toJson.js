...
function makeJSON_(object, options) {
  var reducedObject = object.reduce(function(acc, prev) {
    acc[prev.clave] = prev.texto;
    return acc;
  }, {});
  if (options.format == FORMAT_PRETTY) {
    var jsonString = JSON.stringify(reducedObject, null, 4);
  } else if (options.format == FORMAT_MULTILINE) {
    var jsonString = Utilities.jsonStringify(reducedObject);
